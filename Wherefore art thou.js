...
array of all objects that have matching name and value pairs (second argument). Each name 
and value pair of the source object has to be present in the object from the collection 
...
function whatIsInAName(collection, source) {
    ...
    let collectionFiltered=collection.filter(elementInArray=>{
      let gate=false;
      
      for(let data in source){
        if(elementInArray[data]===source[data]){
          gate=true; 
        }else{
          return gate=false;
        }
